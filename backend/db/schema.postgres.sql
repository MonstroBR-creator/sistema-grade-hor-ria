-- ============================================================
-- SCHEMA — PostgreSQL (produção, ex.: Render)
-- Equivalente a schema.sqlite.sql. Ao alterar um, altere o outro.
...
INSERT INTO turnos (id, codigo, nome) VALUES (1, 'A', 'Manhã') ON CONFLICT DO NOTHING;
INSERT INTO turnos (id, codigo, nome) VALUES (2, 'B', 'Tarde') ON CONFLICT DO NOTHING;
INSERT INTO turnos (id, codigo, nome) VALUES (3, 'C', 'Noite') ON CONFLICT DO NOTHING;

-- Inserir ids explícitos não avança a sequência do SERIAL; sem este acerto, o
-- próximo INSERT automático em turnos tentaria usar o id 1 e falharia.
SELECT setval('turnos_id_seq', (SELECT GREATEST(MAX(id), 1) FROM turnos));
